...
function hostApprovalStatusLabel(status) {
  const map = {
    'Approved': 'Approved',
    'Rejected': 'Rejected',
    'Pending': 'Waiting for Host Approval'
  };
  return map[status] || 'Waiting for Host Approval';
}

function hostApprovalBadge(status) {
  const map = {
  ...
  const cls = map[key] || 'badge-low';
  return `<span class="badge ${cls}">${hostApprovalStatusLabel(key)}</span>`;
}
