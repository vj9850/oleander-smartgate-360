...
function renderAppSidebar(active) {
  ...
  const MASTERS_SET   = new Set(['masterdash','staff','vendor','vehicle','driver','location','invcat','invitems','visitortype','role','gate','event','dept','parking']);
  ...
  sidebar.innerHTML = `
    ...
    <nav class="sidebar-nav">
      ...
      <a href="${p}accountant.html" class="nav-item${a('accountant')}">
      ...
      ${group('masters', 'Master Data',
        ...
        [
          ...
          ['parking',      `${p}masters/parking-master.html`,             'Parking Bays']
        ], mstOpen)}
...
async function clearAppCache() {
  const confirmClear = window.confirm(
    'Clear cached app data? This will reset locally stored records and reload the app. You will stay signed in.'
  );
  ...
  try {
    ...
    if (typeof showToast === 'function') {
      showToast('App cache cleared. Reloading…', 'success');
    }
    ...
    if (typeof showToast === 'function') {
      ...
      window.alert('Could not clear cache');
    }
  }
}
